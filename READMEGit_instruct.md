...
3. Связать удаленный репозиториий с локальным командой `git remote add main https://github.com/lexxxegorov/SCV_Git_1702.git`
Ссылка береться из GitHub
4. Далее используя следующие команды, можно работать с удаленными репозиториями. 

```
git clone - скачиваем полностью копию репозитория в локальный репозиторий. 
git pull - вы обновляете локальную копию новыми коммитами с удаленного репозитория.
git push - вы отправляете изменения на локальном репозитории в удаленный
```
